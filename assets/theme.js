...
slate.rte = {
 ...
 wrapIframe: function (options) {
  options.$iframes.each(function () {
   ...
   $(this).wrap(
    '<div class="video-section"><div class="' +
  ...
  if (this.autorotate) {
   $(document).scroll(
    $.debounce(
     ...
     function () {
      if (
       this.$arrowsMobile.offset().top + this.$arrowsMobile.outerHeight() <
       window.pageYOffset
      ) {
   ...
   this.$dots
    ...
    .each(function () {
     ...
     $dot.on('click keyup', function (evt) {
      if (evt.type === 'keyup' && evt.which !== slate.utils.keyboardKeys.ENTER)
       return;
...
theme.SearchResultsTemplate = (function () {
 ...
 function isOnSale(product) {
  return (
   ...
   parseInt(product.compare_at_price_min, 10) > parseInt(product.price_min, 10)
  );
 }
 ...
 window.theme.SearchHeader = (function () {
  ...
  function init(config) {
   componentInstance = new window.Shopify.theme.PredictiveSearchComponent({
    ...
    onOpen: function (nodes) {
     var searchInputBoundingRect = $(searchInput)[0].getBoundingClientRect();
...
theme.Cart = (function () {
 ...
 Cart.prototype = _.assignIn({}, Cart.prototype, {
   ...
   return $.map(
    ...
    function (discount) {
     ...
     $discount
      ...
      .html(theme.Currency.formatMoney(discount.amount, theme.moneyFormat));
     return $discount[0];
    }.bind(this)
   );
  },
  ...
  _hideQuantityErrorMessage: function () {
   var $errorMessages = $(selectors.cartQuantityErrorMessageWrapper).addClass(
    classes.hide
   );
  ...
  _emptyCart: function () {
   this.$emptyPageContent =
    this.$emptyPageContent || $(selectors.emptyPageContent, this.$container);
   this.$cartWrapper =
   ...
   if (location.search.length) {
    ...
    for (var i = 0; i < aCouples.length; i++) {
     ...
     if (aKeyValue.length > 1) {
      self.queryParams[decodeURIComponent(aKeyValue[0])] = decodeURIComponent(
       aKeyValue[1]
      );
     }
    }
    ...
    .fail(function () {
     ...
     if (Shopify.designMode) {
      $map
       ...
       .html(
        '<div class="' + classes.errorMsg + '">' + errorMessage + '</div>'
       );
     }
    });
  },
 ...
 Product.prototype = _.assignIn({}, Product.prototype, {
  ...
  _initShopifyXrLaunch: function () {
   ...
   $(document).on('shopify_xr_launch', function () {
    var $currentMedia = $(
     self.selectors.productMediaWrapper + ':not(.' + self.classes.hidden + ')',
     self.$container
    );
  ...
  _setCartPopupProductDetails: function (
   ...
   this.$cartPopupProductDetails =
    this.$cartPopupProductDetails || $(this.selectors.cartPopupProductDetails);
  ...
  _setActiveThumbnail: function (mediaId) {
   ...
   var $activeThumbnail = $thumbnailWrappers.find(
    this.selectors.productThumbImages + "[data-thumbnail-id='" + mediaId + "']"
   );
  ...
  _switchMedia: function (mediaId) {
   var $currentMedia = $(
    this.selectors.productMediaWrapper + ':not(.' + this.classes.hidden + ')',
    this.$container
   );
...
$(document1).ready(function () {
 ...
 var background = $('.tab_content .tab_content-wrapper.active a').attr(
  'data-image'
 );
 var overlay_title = $('.tab_content .tab_content-wrapper.active a').attr(
  'data-title'
 );
 var overlay_content = $('.tab_content .tab_content-wrapper.active a').attr(
  'data-content'
 );
 ...
 $('.tab_title-mobile p').click(function () {
  ...
  var background = $(
   '.tab_content-mobile .tab_content-wrapper-mobile.active a'
  ).attr('data-image');
  var overlay_title = $(
   '.tab_content-mobile .tab_content-wrapper-mobile.active a'
  ).attr('data-title');
 ...
 if (cart.item_count == 0) {
  ...
  $.each(cart.items, function (index, item) {
   ...
   cart_items_html +=
    ...
    '<span class="btnqty_ajax qtyminus_ajax" data-line-id="' +
    line_id +
    '">' +
    '-' +
    '</span>' +
    '<input type="text" name="quantity" min="1" class="quantity-selector quantity-input" data-line-id="' +
    line_id +
    '" value="' +
    item.quantity +
    '">' +
    '<span class="btnqty_ajax qtyplus_ajax"  data-line-id="' +
    line_id +
    '">' +
    '+' +
    '</span>' +
    '</div>' +
    '<span class="cart-item-remove" data-line-id="' +
    line_id +
    '">' +
    'Remove' +
    '</span>' +
...
$(document).on('click', '.btnqty_ajax', function () {
 var qty = parseInt(
  $(this).closest('.cart-item__quantity').find('.quantity-input').val()
 );
